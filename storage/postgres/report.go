...
func (r *ReportRepo) GetListBranchShare(ctx context.Context) (*models.GetListBranchShareResponse, error) {
	...
	query := `
		SELECT
			...
			b.id,
			b.name,
			SUM(o.paid_price / 100 * c.investor_percentage) as investor_price
			o.paid_price - investor_price as branch_price
		FROM "order" AS o
		JOIN car AS c ON c.id = o.car_id
		JOIN investor AS i ON i.id = c.investor_id
	...
	for rows.Next() {
		var (
			id            sql.NullString
			name          sql.NullString
			investorPrice sql.NullFloat64
			branchPrice   sql.NullFloat64
		)
		...
		resp.Branches = append(resp.Branches, &models.BranchShare{
			Id:            id.String,
			Name:          name.String,
			InvestorPrice: investorPrice.Float64,
			BranchPrice:   branchPrice.Float64,
		})
	}
